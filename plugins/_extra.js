...
smd({
  cmdname: "inactivekick",
  alias: ["kickinactive", "kicklowmsg"],
  desc: "Kicks users with low message count from the group, excluding admins.",
  category: "group",
  filename: __filename,
  use: "<message_count>"
}, async (_0x5e533c, _0x2a29f6, { store: _0x264360 }) => {
  try {
    ...
    // Vérifier si le bot est administrateur dans le groupe
    ...
    // Vérifier si l'utilisateur a fourni un seuil de messages
    if (!_0x2a29f6 || isNaN(_0x2a29f6)) {
      return _0x5e533c.reply("*_Please provide a valid number of messages as the threshold for inactivity._*");
    }

    const messageThreshold = parseInt(_0x2a29f6); // Seuil de messages
    let userMessageCount = {}; // Stockage du nombre de messages par utilisateur
    
    // Récupérer l'historique des messages et compter les messages par utilisateur
    _0x264360.messages[_0x5e533c.jid].array.forEach(msg => {
      const user = msg.pushName || (_0x5e533c.isGroup ? msg.key.participant : msg.key.remoteJid || "unknown").split("@")[0];
      userMessageCount[user] = (userMessageCount[user] || 0) + 1;
    });

    // Filtrer les utilisateurs qui ont envoyé moins que le seuil de messages
    let inactiveUsers = [];
    for (let user in userMessageCount) {
      if (userMessageCount[user] < messageThreshold && !await isAdmin(user, _0x5e533c)) {
        inactiveUsers.push(user);
      }
    }

    if (inactiveUsers.length === 0) {
      return _0x5e533c.reply("*_No users found with less than " + messageThreshold + " messages._*");
    }

    // Retirer les utilisateurs inactifs du groupe
    await _0x5e533c.bot.groupParticipantsUpdate(_0x5e533c.chat, inactiveUsers.map(user => `${user}@s.whatsapp.net`), "remove");
    await _0x5e533c.reply("*_The following users have been removed for inactivity (less than " + messageThreshold + " messages):_*\n" + inactiveUsers.join("\n"));
  } catch (_0x4bf40a) {
    await _0x5e533c.error(_0x4bf40a + "\n\ncommand: inactivekick", _0x4bf40a, "*_Error occurred while kicking inactive users!_*");
  }
});

// Fonction pour vérifier si l'utilisateur est un administrateur
async function isAdmin(user, _0x5e533c) {
  const groupMetadata = await _0x5e533c.bot.groupMetadata(_0x5e533c.chat);
  const admins = groupMetadata.participants.filter(participant => participant.isAdmin);
  return admins.some(admin => admin.jid === `${user}@s.whatsapp.net`);
}
});
smd({
  cmdname: "caption",
